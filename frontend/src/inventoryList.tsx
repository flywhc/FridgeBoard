...
export function InventoryList({ inventory, icons, categories = [], title, slotId, slot, refrigerator, refrigeratorByItemId, layoutsByRefrigeratorId, onSelectFridge, onRenameSlot, initialQuery, expiryStatus, summaryLabel, loading = false, error = '', warning = '', emptyMessage, onBack, onAdd, onSelect, onSaveQuantity, onMoveSelected, onDeleteSelected, onClassifySelected, onAddGroup, onAddSubcategory }: {
  ...
  const classifySelected = async (category: Category): Promise<boolean> => {
    if (classifying || !onClassifySelected || !canClassifySelected) return false
    setClassifying(true)
    setClassifyError('')
    ...
    <section className="p5-inventory-items" aria-live="polite">
      {!loading && !error && items.map(item => {
        ...
        return <article className={`p5-inventory-item ${isEmpty ? 'is-empty' : ''}`} key={item.id}>
          <button className={`p5-icon-circle p5-inventory-select ${isSelected ? 'is-selected' : ''}`} type="button" aria-pressed={isSelected} aria-label={isSelected ? '取消选择' : '选择物品'} onClick={() => toggleSelection(item.id)}><CategoryIcon iconKey={item.icon_key} icons={icons} label={item.item_name} />{isSelected && <span className="p5-inventory-select-check" aria-hidden="true"><svg viewBox="0 0 24 24"><path d="m5 12 4 4L19 6" /></svg></span>}</button>
