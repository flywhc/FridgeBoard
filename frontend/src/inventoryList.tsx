...
export function InventoryList({ inventory, icons, categories = [], title, slotId, slot, refrigerator, refrigeratorByItemId, layoutsByRefrigeratorId, onSelectFridge, onRenameSlot, initialQuery, expiryStatus, summaryLabel, loading = false, error = '', emptyMessage, onBack, onAdd, onSelect, onSaveQuantity, onMoveSelected, onDeleteSelected, onClassifySelected, onAddGroup, onAddSubcategory }: {
  ...
  const footer = selectedItems.length ? <footer className={`bottom-action-bar p5-selection-actions${canDeleteSelected ? ' has-delete' : ''}${canClassifySelected ? ' has-category' : ''}`}><button className="p5-selection-cancel" type="button" onClick={cancelSelection}>取消</button><button className="p5-selection-move" type="button" onClick={moveSelected}>移动</button>{canClassifySelected && <button className="p5-selection-category" type="button" onClick={openClassifyDialog}>分类</button>}{canDeleteSelected && <button className="p5-selection-delete" type="button" onClick={openDeleteDialog}>删除</button>}</footer> : onAdd && <footer className="bottom-action-bar"><button className="p5-add-item" type="button" onClick={onAdd}>＋ 添加物品</button></footer>
