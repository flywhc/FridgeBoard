...
import { OrderRecognitionList, RecognitionProgress } from './inventoryRecognitionComponents'
export { OrderRecognitionList, RecognitionProgress } from './inventoryRecognitionComponents'
...
export function InventoryFlow({ layout, categories, icons, inventory, refrigerator, saving, initialSlotId, initialItemId, initialView = 'add', initialExpiryStatus, onBack, onSelectFridge, onRenameSlot, onCatalogChanged, onSave, onDelete, onMoveSelected, onDeleteSelected, onClassifySelected }: {
  ...
  const saveFromLocation = async (slotId = draft.slotId) => {
    ...
    window.setTimeout(() => { locationSubmittingRef.current = false; setAddAnimation(false); setLocationSubmitting(false); setLocationOpen(false); resetDraft(); setNotice(''); if (initialView === 'recognition' || initialView === 'edit') { flowHistoryRef.current = []; onBack() } else if (initialView === 'list') finishFlow('list') }, 550)
  }
  ...
  const openRecognition = () => {
    ...
    const notice = !window.isSecureContext
      ...
      : !navigator.mediaDevices?.getUserMedia
        ...
        : ''
    setNotice(notice); setBarcode(''); setOrderItems([]); setOrderSelection({}); setOrderCategoryIndex(null); setOrderLocationIndex(null); setCameraReady(false); setCameraOpen(!notice); navigateTo('recognition')
  }
  const closeRecognition = () => {
    recognitionErrorRef.current = false
    setRecognitionError('')
    closeCameraView()
    restorePreviousView(onBack)
  }
  ...
  if (view === 'edit') return <PageShell key="edit" className="p5-flow" header={<PageHeader title="编辑物品" onBack={backFrom} right={<button className="p5-header-action" type="button" onClick={() => void save()} disabled={saving} aria-label="保存物品" title="保存物品"><SaveIcon /></button>} />} bodyClassName="p5-scroll p5-edit">
    ...
    <div className="p5-date-row"><DatePickerField label="生产日期" value={draft.productionDate} onChange={productionDate => update({ productionDate })} /><DatePickerField label="保质期至（可选）" value={draft.bestBefore} onChange={bestBefore => { setBestBeforeChanged(true); update({ bestBefore }) }} /></div>
    ...
    <button className="p5-row-link p5-slot-link" onClick={() => navigateTo('location')}><span><small>存放位置</small><b>{selectedSlot ? formatStorageSlotLabel(selectedSlot.zone.label, selectedSlot.key, selectedSlot.custom_name) : '请选择'}</b></span><i>›</i></button>
    <button className="p5-delete" onClick={() => void onDelete(draft.id).then(deleted => { if (deleted) { if (initialView === 'edit') { flowHistoryRef.current = []; onBack(); return }; finishFlow(returnToList ? 'list' : 'add'); setNotice(returnToList ? '' : '物品已删除。') } })}>删除物品</button>
  </PageShell>
  ...
  return <PageShell className="p5-flow" header={<PageHeader title="添加物品" onBack={backFrom} right={<button className="p6-scan-button" type="button" onClick={openRecognition} aria-label="打开扫码和物品识别"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M4 9V5h4M20 9V5h-4M4 15v4h4M20 15v4h-4" /><path d="M7 12h10" /></svg></button>} />} bodyClassName="p5-scroll p5-add" footer={<footer className="bottom-action-bar"><button className="p5-add-item" onClick={() => void advance()}>加入冰箱</button></footer>}>
    ...
    <div className="p5-date-row"><DatePickerField label="生产日期" value={draft.productionDate} onChange={productionDate => update({ productionDate })} /><DatePickerField label="保质期至（可不填）" value={draft.bestBefore} onChange={bestBefore => { setBestBeforeChanged(true); update({ bestBefore }) }} /></div>
