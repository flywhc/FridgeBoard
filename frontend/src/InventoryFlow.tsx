...
export function InventoryFlow({ layout, categories, icons, inventory, saving, initialSlotId, initialView = 'add', onBack, onCreateCategory, onCatalogChanged, onSave, onDelete }: {
  ...
  useEffect(() => {
    ...
    const updateCatalogTop = () => setCatalogTop(
      Math.max(0, catalogRef.current?.getBoundingClientRect().top ?? 0),
    )
  ...
  const openCatalog = () => {
    setCatalogTop(Math.max(0, catalogRef.current?.getBoundingClientRect().top ?? 0))
  ...
  const confirmGeneratedIcon = async () => {
    ...
    try {
      ...
      await onCatalogChanged(); update({ subcategoryId: created.id, itemName: draft.itemName || created.name }); setGeneration(null); setView('add')
  ...
  const backFrom = () => { if (view === 'location') setView('edit'); else if (view === 'edit') setView(returnToList ? 'list' : 'add'); else if (view === 'custom') { cancelGeneratedIcons(); setView('add') } else onBack() }
  ...
  if (view === 'edit') return <PageShell className="p5-flow" header={<PageHeader title="编辑物品" onBack={backFrom} right={<button className="save-text" onClick={() => void save()} disabled={saving}>保存</button>} />} bodyClassName="p5-scroll p5-edit">
    ...
    <div className="p5-row-link"><span><small>小类</small><b>{selectedChild?.name}</b></span></div>
  ...
  return <PageShell className="p5-flow" header={<PageHeader title="添加物品" onBack={backFrom} right={<button className="p6-scan-button" type="button" onClick={openRecognition} aria-label="打开扫码和物品识别"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M4 9V5h4M20 9V5h-4M4 15v4h4M20 15v4h-4" /><path d="M7 12h10" /></svg></button>} />} bodyClassName="p5-scroll p5-add" footer={<footer className="bottom-action-bar"><button onClick={() => void advance()}>加入冰箱</button></footer>}>
    ...
    <section ref={catalogRef} className="p5-catalog">{catalogExpanded ? <div className="p5-catalog-panel" role="dialog" aria-modal="true" aria-label="选择物品" style={{ top: `${catalogTop}px` }}><div className="p5-catalog-dialog-heading"><strong>选择物品</strong><label><svg className="p5-search-icon" viewBox="0 0 24 24" aria-hidden="true"><circle cx="10.5" cy="10.5" r="6.5" /><path d="m16 16 5 5" /></svg><input value={query} onChange={event => setQuery(event.target.value)} placeholder="搜索全部小类" aria-label="搜索全部小类" /></label><button type="button" onClick={() => setCatalogExpanded(false)} aria-label="关闭选择物品">×</button></div><div className="p5-catalog-body"><aside>{parents.map(group => <button className={group.id === activeGroupId ? 'is-selected' : ''} key={group.id} onClick={() => setActiveGroupId(group.id)}>{group.name}</button>)}<button className="p5-add-group" onClick={openGroupDialog}>＋ 添加大类</button></aside><div className="p5-catalog-items"><div className="p5-icon-grid">{matchingChildren.map(child => <button className={child.id === draft.subcategoryId ? 'is-selected' : ''} key={child.id} onClick={() => chooseChild(child)}><span><CategoryIcon iconKey={child.icon_key} icons={icons} label={child.name} /></span><b>{child.name}</b></button>)}</div><button className="p5-new-subcategory" onClick={() => { setCustomName(''); setGeneration(null); setView('custom') }}>＋ 新建小类</button></div></div></div> : <><div className="p5-catalog-heading"><span>选择物品</span><label><svg className="p5-search-icon" viewBox="0 0 24 24" aria-hidden="true"><circle cx="10.5" cy="10.5" r="6.5" /><path d="m16 16 5 5" /></svg><input value={query} onChange={event => setQuery(event.target.value)} placeholder="搜索全部小类" aria-label="搜索全部小类" /></label><button type="button" onClick={openCatalog} aria-label="展开选择物品"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="m6 10 6 6 6-6" /></svg></button></div><div className="p5-parent-grid">{(query.trim() ? matchingChildren : recentDisplayCategories).map(child => <button className={child.id === draft.subcategoryId ? 'is-selected' : ''} key={child.id} onClick={() => chooseChild(child)}><CategoryIcon iconKey={child.icon_key} icons={icons} label={child.name} /><b>{child.name}</b></button>)}</div></>}</section>
    {groupDialogOpen && <div className="p5-group-modal" role="dialog" aria-modal="true" aria-labelledby="p5-group-dialog-title"><form className="p5-group-dialog" onSubmit={event => { event.preventDefault(); void createGroup() }}><button type="button" className="p5-group-close" onClick={() => setGroupDialogOpen(false)} disabled={creatingGroup} aria-label="关闭添加大类">×</button><h2 id="p5-group-dialog-title">添加大类</h2><p>为物品选择器新增一个导航大类。</p><label className="p5-group-field"><span>大类名称</span><input autoFocus value={groupName} maxLength={80} onChange={event => { setGroupName(event.target.value); setGroupError('') }} placeholder="请输入名称" disabled={creatingGroup} /></label>{groupError && <p className="p5-group-error" role="alert">{groupError}</p>}<div className="p5-group-actions"><button type="button" onClick={() => setGroupDialogOpen(false)} disabled={creatingGroup}>取消</button><button type="submit" disabled={creatingGroup}>{creatingGroup ? '添加中…' : '添加大类'}</button></div></form></div>}
