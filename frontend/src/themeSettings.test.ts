...
describe('主题设置页面', () => {
  it('应用偏好显示当前主题并进入主题设置', () => {
    const markup = renderToStaticMarkup(createElement(ThemePreferencesPage, { theme: 'cartoon', onBack: () => undefined, onOpenThemeSettings: () => undefined }))
    expect(markup).toContain('应用偏好')
    expect(markup).toContain('<b>主题</b><small>卡通</small>')
    expect(markup).toContain('这些设置只保存在当前设备。')
  })
