...
export function RecipeWorkspace({ refrigerator, icons, inventory, refreshNonce, onBack, onFridge, onMe, onInventoryChanged }: { refrigerator: Refrigerator; icons: Icon[]; inventory: InventoryBatch[]; refreshNonce: number; onBack: () => void; onFridge: () => void; onMe: () => void; onInventoryChanged: () => Promise<void> }) {
  ...
  if (view === 'restock') return <PageShell className="p7-shell p9-shell" header={<PageHeader title="补货清单" onBack={() => setView('week')} right={<button className="p7-icon-button p9-copy-button" onClick={() => void copyRestock()} aria-label="复制补货清单" title="复制补货清单"><svg viewBox="0 0 24 24" aria-hidden="true"><rect x="8" y="8" width="11" height="12" rx="1" /><path d="M16 8V5a2 2 0 0 0-2-2H5a2 2 0 0 0-2 2v11h3" /></svg></button>} />} bodyClassName="p7-scroll p9-list">{copyNotice && <p className={`p9-copy-notice ${copyNotice === '已复制到剪切板' ? 'is-success' : ''}`} role="status" aria-live="polite">{copyNotice}</p>}{restock.length ? restock.map(item => <section key={`${item.weekday}-${item.dish_name}`}><h2>{item.label} · {item.dish_name}</h2><RestockMissingLine missing={item.missing} /></section>) : <p className="p9-empty">本周和下周食材都足够。</p>}</PageShell>
