...
export function RecipeWorkspace({ refrigerator, icons, inventory, refreshNonce, onBack, onFridge, onMe, onInventoryChanged }: { refrigerator: Refrigerator; icons: Icon[]; inventory: InventoryBatch[]; refreshNonce: number; onBack: () => void; onFridge: () => void; onMe: () => void; onInventoryChanged: () => Promise<void> }) {
  ...
  }) : canEditRecipes ? <button className="p9-empty p9-empty-action" onClick={() => { setEditing({ id: '', weekday: day.weekday, dish_name: '', method: null, note: null, completed: false, ingredients: [], missing: [] }); setView('edit') }} aria-label={`${day.label}添加食谱`}>＋ 添加食谱</button> : <p className="p9-empty">还没有安排</p>}</section>)}</PageShell>
}
