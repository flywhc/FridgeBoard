...
export function RestockMissingLine({ missing, inventory = [], icons = [], categories = [] }: { missing: RestockEntry['missing']; inventory?: Pick<InventoryBatch, 'item_name' | 'icon_key'>[]; icons?: Icon[]; categories?: Pick<Category, 'id' | 'icon_key'>[] }) {
  const theme = useTheme()
  return <p className="p9-restock-missing"><b>{missing.map((item, index) => {
    const categoryIconKey = item.subcategory_id ? categories.find(category => category.id === item.subcategory_id)?.icon_key : null
    const icon = icons.find(candidate => candidate.key === categoryIconKey) ?? getRecipeIngredientIcon(item.subcategory_name, inventory, icons)
...
export function CustomShoppingList({ items, categories = [], icons = [] }: { items: CustomShoppingItem[]; categories?: Pick<Category, 'id' | 'icon_key'>[]; inventory?: Pick<InventoryBatch, 'item_name' | 'icon_key'>[]; icons?: Icon[] }) {
  const theme = useTheme()
  return <p className="p9-custom-shopping-list">{items.map((item, index) => {
    const categoryIconKey = item.subcategory_id ? categories.find(category => category.id === item.subcategory_id)?.icon_key : null
    const icon = categoryIconKey ? icons.find(candidate => candidate.key === categoryIconKey) : undefined
    const resolved = icon ? resolveIconVariant(icon, theme) : null
    return <span className="p9-restock-item" key={item.id}>{resolved && <RuntimeImage className="p9-restock-item-icon" src={resolved.assetUrl} alt="" />}<span>{item.item_name} × {item.quantity}</span>{index < items.length - 1 && '，'}</span>
  })}</p>
}
...
export function RecipeWorkspace({ refrigerator, categories = [], icons, inventory, refreshAt, initialView = 'week', onBack, onMe, onInventoryChanged, onPrioritizeRefresh, refreshGeneration = pageRefreshGuard.currentGeneration() }: { refrigerator: Refrigerator; categories?: Category[]; icons: Icon[]; inventory: InventoryBatch[]; refreshAt: number; initialView?: 'week' | 'restock'; onBack: () => void; onMe: () => void; onInventoryChanged: () => Promise<void>; onPrioritizeRefresh?: (key: string) => Promise<void> | null; refreshGeneration?: number }) {
  ...
  const selectWeek = useCallback((offset: 0 | 7) => {
