...
import { InstallationGuide, OptionPickerField, PageHeader, PageShell } from './sharedUi'
...
export function BootstrapPairing({ token, kind = 'bootstrap', onScan, targetRefrigeratorId, displayBindingPurpose, onContinueSetup, onOpenHome }: {
  ...
  if (mode === 'signed-out') return <PageShell className="claim-screen" header={<PageHeader title="连接冰箱" />} bodyClassName="claim-content"><p>{kind === 'grant_pwa_access' ? '登录不是必需的；登录后也可以保留本次扫码意图。' : '登录后可选择已有冰箱，或新建一台冰箱。'}</p>{message && <p role="alert" className="claim-error">{message}</p>}<button className="p7-primary" type="button" onClick={login}>登录或注册</button><button className="p7-outline secondary-action scan-entry" type="button" onClick={onScan}>扫描新的二维码</button>{kind === 'grant_pwa_access' && <button className="p7-primary" type="button" disabled={consuming} onClick={() => void consume()}>{consuming ? '连接中…' : '继续连接'}</button>}</PageShell>
  return <PageShell className="claim-screen" header={<PageHeader title={kind === 'grant_pwa_access' ? '连接冰箱' : '连接这台冰箱'} />} bodyClassName="claim-content"><p>{kind === 'grant_pwa_access' ? '扫描已配置冰箱的二维码后，这台手机即可获得日常访问权限。' : '二维码仍有效时，选择目标冰箱并完成绑定。'}</p>{message && <p role="alert" className="claim-error">{message}</p>}
    {kind === 'grant_pwa_access' ? <button className="p7-primary" type="button" disabled={consuming} onClick={() => void consume()}>{consuming ? '连接中…' : '连接冰箱'}</button> : <form onSubmit={consume}>{targetRefrigeratorId ? <p>将绑定到：{fridges.find(fridge => fridge.id === targetRefrigeratorId)?.name ?? '目标冰箱'}</p> : fridges.length ? <OptionPickerField label="选择冰箱" value={selectedId} options={[{ value: '', label: '新建一台冰箱' }, ...fridges.map(fridge => ({ value: fridge.id, label: fridge.name }))]} onChange={setSelectedId} /> : null}{!selectedId && !targetRefrigeratorId && <label>冰箱名称<input value={newName} onChange={event => setNewName(event.target.value)} required maxLength={120} /></label>}<button className="p7-primary" type="submit" disabled={consuming}>{consuming ? '连接中…' : '连接冰箱'}</button></form>}
    <button className="p7-outline secondary-action scan-entry" type="button" onClick={onScan}>扫描新的二维码</button>
  </PageShell>
}
