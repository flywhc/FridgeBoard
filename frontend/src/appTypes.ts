...
export type InventoryBatch = { id: string; subcategory_id: string; subcategory_name: string; icon_key: string | null; storage_slot_id: string; storage_slot_name?: string | null; item_name: string; quantity: number; production_date: string | null; best_before: string | null; product_description: string | null; price?: string | null; barcode: string | null; expiry_status: string | null; updated_at?: string }
import type { ThemeKey } from './theme'

export type IconVariant = { asset_url: string; media_type: 'image/svg+xml' | 'image/png'; source?: string | null; source_url?: string | null; attribution?: string | null }
export type Icon = { key: string; label: string; asset_url: string; media_type?: 'image/svg+xml' | 'image/png'; fallback_theme?: ThemeKey; variants?: Partial<Record<ThemeKey, IconVariant>> }
...
export type RecognitionOrderItem = { item_name: string; specification: string; quantity: number; price?: string | null; subcategory_id?: string; subcategory_name?: string; subcategory_confidence?: number; storage_slot_id?: string }
...
export type CategoryRecognitionItem = { item_name: string; source: 'inventory' | 'recipe' | 'shopping' | 'current' }
export type CategoryRecognitionResult = { category_id: string; category_name: string; items: CategoryRecognitionItem[] }
...
export type IconCandidate = { id: string; asset_url: string; media_type?: 'image/svg+xml' | 'image/png' }
...
export type CustomShoppingItem = { id: string; item_name: string; quantity: number; display_order: number; subcategory_id?: string | null }
