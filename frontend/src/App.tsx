...
export function FridgeHome({ refrigerator, layout, homeInventory, icons, notice, notifications, refreshState, refreshError, installEvent, installed, onInstallEventConsumed, onAdd, onInventory, onExpiring, onExpired, onSlot, onManage, onFridgeList, onSwipeFridge, fridgeSwipeTransition, onRefresh, onRecipes, onShopping, onMe, onSearch }: { refrigerator: Refrigerator; layout: Layout; homeInventory: InventoryBatch[]; icons: Icon[]; notice: string; notifications: DueNotification[]; refreshState: RefreshState; refreshError: string; installEvent: BeforeInstallPromptEvent | null; installed: boolean; onInstallEventConsumed: () => void; onAdd: () => void; onInventory: () => void; onExpiring: () => void; onExpired: () => void; onSlot: (slotId: string) => void; onManage: () => void; onFridgeList: () => void; onSwipeFridge?: (direction: HorizontalSwipeDirection) => void; fridgeSwipeTransition?: { direction: HorizontalSwipeDirection; phase: FridgeSwipeTransitionPhase } | null; onRefresh: () => void; onRecipes: () => void; onShopping: () => void; onMe: () => void; onSearch: (query: string) => void }) {
  ...
  return <PageShell className="p7-shell p7-top-level" onRefresh={onRefresh} refreshState={refreshState} header={<AppHeader title={<HeaderTitle title={refrigerator.name} refreshState={refreshState} refreshError={refreshError} />} left={<button className="p7-icon-button" onClick={onManage} aria-label="管理冰箱">☰</button>} right={<button className="p7-icon-button" onClick={onFridgeList} aria-label="查看我的冰箱"><svg className="p7-fridge-outline-icon" data-icon="solar:fridge-outline" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><path d="M6.5 3.5h11A2.5 2.5 0 0 1 20 6v13.5a1 1 0 0 1-1 1H5a1 1 0 0 1-1-1V6a2.5 2.5 0 0 1 2.5-2.5Z" /><path d="M4 9.5h16M8 6v1M8 12v4" /></svg></button>} />} bodyClassName="p7-home-content" footer={<P7Navigation active="home" onHome={() => undefined} onRecipes={onRecipes} onShopping={openShopping} onMe={onMe} />}>
    ...
    <div className="p7-status"><button className="p7-inventory-summary" type="button" onClick={onInventory} aria-label={`查看全部 ${inventoryCount} 件物品`}><svg className="p7-inventory-icon" viewBox="0 0 24 24" aria-hidden="true"><path d="M5.5 10.5c0-2.4 1.7-4 4-4 1 0 1.9.4 2.5 1 .6-.6 1.5-1 2.5-1 2.3 0 4 1.6 4 4 0 4.1-2.3 8-6.5 8s-6.5-3.9-6.5-8Z" /><path d="M12 7.5c-.1-1.8.8-3-1.3-4.5M13.2 4.5c1.2-.1 2.4-.7 3.1-1.7" /></svg>{inventoryCount} 件物品 <span aria-hidden="true">›</span></button><form className="p7-inventory-search" onSubmit={event => { event.preventDefault(); submitSearch() }}><svg className="p7-search-icon" viewBox="0 0 24 24" aria-hidden="true"><circle cx="10.5" cy="10.5" r="6.5" /><path d="m16 16 5 5" /></svg><input value={searchQuery} onChange={event => setSearchQuery(event.target.value)} placeholder="搜索所有冰箱" aria-label="搜索所有冰箱的物品" /></form>{(notice || notifications.length > 0) && <span className="p7-status-actions"><button className="p7-icon-button p7-status-notice" onClick={() => setIsNoticeOpen(true)} aria-label={notifications.length > 0 ? `查看 ${notifications.length} 条通知` : '查看首页提示'} aria-haspopup="dialog">!</button></span>}<span className="p7-risk-summaries">{expiring > 0 && <button className="p7-risk-summary" type="button" onClick={onExpiring} aria-label={`查看 ${expiring} 件临期物品`}><RiskSummaryIcon status="expiring" /><span className="p7-risk-count">{expiring}</span></button>}{expired > 0 && <button className="p7-risk-summary" type="button" onClick={onExpired} aria-label={`查看 ${expired} 件过期物品`}><RiskSummaryIcon status="expired" /><span className="p7-risk-count">{expired}</span></button>}</span></div>
