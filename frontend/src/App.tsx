...
import { AppHeader, CategoryIcon, ConfirmDialog, Dialog, HeaderTitle, InstallationGuide, NoticeDialog, P7Navigation, PageHeader, PageShell, type RefreshState } from './sharedUi'
...
function RiskSummaryIcon({ status }: { status: InventoryExpiryStatus }) {
  if (status === 'expiring') {
    return <svg className="p7-risk-icon" data-icon="iconoir:clock" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><path d="M12 6v6h6" /><path d="M12 22c5.523 0 10-4.477 10-10S17.523 2 12 2S2 6.477 2 12s4.477 10 10 10" /></svg>
  }
  return <svg className="p7-risk-icon" data-icon="ant-design:warning-outlined" viewBox="0 0 1024 1024" aria-hidden="true"><path d="M464 720a48 48 0 1 0 96 0 48 48 0 1 0-96 0m16-304v184c0 4.4 3.6 8 8 8h48c4.4 0 8-3.6 8-8V416c0-4.4-3.6-8-8-8h-48c-4.4 0-8 3.6-8 8m475.7 440-416-720c-6.2-10.7-16.9-16-27.7-16s-21.6 5.3-27.7 16l-416 720C56 877.4 71.4 904 96 904h832c24.6 0 40-26.6 27.7-48m-783.5-27.9L512 239.9l339.8 588.2z" /></svg>
}

export function FridgeHome({ refrigerator, layout, homeInventory, icons, notifications, refreshState, refreshError, installEvent, installed, onInstallEventConsumed, onAdd, onInventory, onExpiring, onExpired, onSlot, onFridgeList, onSwipeFridge, fridgeSwipeTransition, onRefresh, onRecipes, onShopping, onMe, onSearch }: { refrigerator: Refrigerator; layout: Layout; homeInventory: InventoryBatch[]; icons: Icon[]; notifications: DueNotification[]; refreshState: RefreshState; refreshError: string; installEvent: BeforeInstallPromptEvent | null; installed: boolean; onInstallEventConsumed: () => void; onAdd: () => void; onInventory: () => void; onExpiring: () => void; onExpired: () => void; onSlot: (slotId: string) => void; onFridgeList: () => void; onSwipeFridge?: (direction: HorizontalSwipeDirection) => void; fridgeSwipeTransition?: { direction: HorizontalSwipeDirection; phase: FridgeSwipeTransitionPhase } | null; onRefresh: () => void; onRecipes: () => void; onShopping: () => void; onMe: () => void; onSearch: (query: string) => void }) {
  ...
  const expired = activeHomeInventory.filter(item => item.expiry_status === 'expired').length
  ...
  return <PageShell className="p7-shell p7-top-level" onRefresh={onRefresh} refreshState={refreshState} header={<AppHeader title={<HeaderTitle title={refrigerator.name} refreshState={refreshState} refreshError={refreshError} />} left={<button type="button" className="p7-icon-button" onClick={onFridgeList} aria-label="查看我的冰箱"><svg className="p7-fridge-outline-icon" data-icon="solar:fridge-outline" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><path d="M6.5 3.5h11A2.5 2.5 0 0 1 20 6v13.5a1 1 0 0 1-1 1H5a1 1 0 0 1-1-1V6a2.5 2.5 0 0 1 2.5-2.5Z" /><path d="M4 9.5h16M8 6v1M8 12v4" /></svg></button>} right={<button type="button" className="p7-icon-button p9-add-shopping-button" onClick={onAdd} aria-label="添加物品"><svg data-icon="lucide:plus" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" aria-hidden="true"><path d="M12 5v14M5 12h14" /></svg></button>} />} bodyClassName="p7-home-content" footer={<P7Navigation active="home" onHome={() => undefined} onRecipes={onRecipes} onShopping={openShopping} onMe={onMe} />}>
    <PwaInstallPrompt installEvent={installEvent} installed={installed} onInstallEventConsumed={onInstallEventConsumed} />
    <div className="p7-status"><button className="p7-inventory-summary" type="button" onClick={onInventory} aria-label={`查看全部 ${inventoryCount} 件物品`}><svg className="p7-inventory-icon" viewBox="0 0 24 24" aria-hidden="true"><path d="M5.5 10.5c0-2.4 1.7-4 4-4 1 0 1.9.4 2.5 1 .6-.6 1.5-1 2.5-1 2.3 0 4 1.6 4 4 0 4.1-2.3 8-6.5 8s-6.5-3.9-6.5-8Z" /><path d="M12 7.5c-.1-1.8.8-3-1.3-4.5M13.2 4.5c1.2-.1 2.4-.7 3.1-1.7" /></svg>{inventoryCount} 件物品 <span aria-hidden="true">›</span></button><form className="p7-inventory-search" onSubmit={event => { event.preventDefault(); submitSearch() }}><svg className="p7-search-icon" viewBox="0 0 24 24" aria-hidden="true"><circle cx="10.5" cy="10.5" r="6.5" /><path d="m16 16 5 5" /></svg><input value={searchQuery} onChange={event => setSearchQuery(event.target.value)} placeholder="搜索所有冰箱" aria-label="搜索所有冰箱的物品" /></form>{notifications.length > 0 && <span className="p7-status-actions"><button className="p7-icon-button p7-status-notice" onClick={() => setIsNoticeOpen(true)} aria-label={`查看 ${notifications.length} 条通知`} aria-haspopup="dialog">!</button></span>}<span className="p7-risk-summaries">{expiring > 0 && <button className="p7-risk-summary" type="button" onClick={onExpiring} aria-label={`查看 ${expiring} 件临期物品`}><RiskSummaryIcon status="expiring" /><span className="p7-risk-count">{expiring}</span></button>}{expired > 0 && <button className="p7-risk-summary" type="button" onClick={onExpired} aria-label={`查看 ${expired} 件过期物品`}><RiskSummaryIcon status="expired" /><span className="p7-risk-count">{expired}</span></button>}</span></div>
    {notifications.length > 0 && isNoticeOpen && <NoticeDialog title="首页提示" message={<>{notifications.map(item => <p key={`${item.kind}-${item.title}`}>{item.title}：{item.body}</p>)}</>} onClose={() => setIsNoticeOpen(false)} />}
...
function MeHome({ theme, onNotifications, onAbout, onPreferences, onHome, onRecipes, onShopping, onSwitchAccount }: { theme: ThemeKey; onNotifications: () => void; onAbout: () => void; onPreferences: () => void; onHome: () => void; onRecipes: () => void; onShopping: () => void; onSwitchAccount?: () => void }) {
  ...
  return <>
    <PageShell className="p7-shell p7-top-level" header={<AppHeader title="我的" />} bodyClassName="p7-scroll p7-settings" footer={<P7Navigation active="me" onHome={onHome} onRecipes={onRecipes} onShopping={openShopping} onMe={() => undefined} />}>
...
export function App() {
  ...
  return <FridgeHome refrigerator={currentFridge} layout={layout} homeInventory={homeInventory} icons={icons} notifications={dueNotifications} refreshState={refreshState} refreshError={refreshError} installEvent={installEvent} installed={pwaInstalled} onInstallEventConsumed={() => setInstallEvent(null)} onAdd={() => { setInventorySlotId(undefined); setInventoryItemId(undefined); setInventoryExpiryStatus(undefined); setInventoryMode('add'); setP7View('inventory') }} onInventory={() => { setInventorySlotId(undefined); setInventoryItemId(undefined); setInventoryExpiryStatus(undefined); setInventoryMode('list'); setP7View('inventory') }} onExpiring={() => { setInventorySlotId(undefined); setInventoryItemId(undefined); setInventoryExpiryStatus('expiring'); setInventoryMode('list'); setP7View('inventory') }} onExpired={() => { setInventorySlotId(undefined); setInventoryItemId(undefined); setInventoryExpiryStatus('expired'); setInventoryMode('list'); setP7View('inventory') }} onSlot={slotId => { setInventorySlotId(slotId); setInventoryItemId(undefined); setInventoryExpiryStatus(undefined); setInventoryMode('list'); setP7View('inventory') }} onFridgeList={() => setP7View('switcher')} onSwipeFridge={swipeHomeFridge} fridgeSwipeTransition={fridgeSwipeTransition} onRefresh={() => loadInventoryWorkspace(currentFridge, true)} onRecipes={() => setP7View('recipes')} onShopping={() => setP7View('shopping')} onMe={() => setP7View('me')} onSearch={query => { setSearchQuery(query); setP7View('search') }} />
}
