...
vi.mock('./sharedUi', () => ({
  ...
  OptionPickerDialog: ({ title, value, options, onChange, onClose }: { title: string; value: string; options: Array<{ value: string; label: string }>; onChange: (value: string) => void; onClose: () => void }) => createElement('section', { role: 'dialog', 'aria-modal': 'true' }, createElement('h2', null, title), createElement('button', { type: 'button', onClick: onClose }, '关闭'), options.map(option => createElement('button', { key: option.value, type: 'button', role: 'option', 'aria-selected': option.value === value, onClick: () => { onChange(option.value); onClose() } }, option.label))),
