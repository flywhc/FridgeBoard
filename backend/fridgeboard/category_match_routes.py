...
def _candidate_payload(session: Session, refrigerator_id: str) -> list[dict[str, object]]:
...
def _deterministic_match(
    ...
    _purge_expired_ai_mappings(session, now=now)
    ...
    if (
        ...
        category = session.get(FoodCategory, mapping.subcategory_id)
        if category is not None and category.parent_id is not None and category.refrigerator_id in {
            None,
            refrigerator_id,
        }:
            return _as_match_result(mapping, category)

    candidates = _candidate_payload(session, refrigerator_id)
...
def register_category_match_routes(
    ...
    *,
    ...
    def owner_category_match(
        refrigerator_id: str,
        payload: CategoryMatchRequest,
        current_owner: str = Depends(owner_context.owner_id),
    ) -> CategoryMatchResponse:
        """执行所有者范围内的快速分类匹配。"""
        with owner_context.transaction(owner_context.session_factory) as session:
            ...
            result = _deterministic_match(session, refrigerator_id, payload.item_name)
    ...
    async def owner_cancel_category_match(
        ...
        """取消所有者范围内的分类请求。"""
        with owner_context.session_factory() as session:
            refrigerator = session.get(Refrigerator, refrigerator_id)
            if (
                refrigerator is None
                or refrigerator.owner_user_id != current_owner
    ...
    def daily_category_match(
        ...
        with daily_context.transaction(daily_context.session_factory) as session:
            result = _deterministic_match(session, refrigerator_id, payload.item_name)
        request_id = uuid.uuid4().hex
        if result is not None:
...
async def _run_ai(
    ...
    with context.transaction(context.session_factory) as session:
        if isinstance(actor, str):
            ...
            _authorize_daily(context.session_factory, refrigerator_id, actor)
        candidates = _candidate_payload(session, refrigerator_id)
    ...
    try:
        ...
        confidence = 0.5
    with context.transaction(context.session_factory) as session:
        _purge_expired_ai_mappings(session)
        ...
        if mapping is None:
            ...
            confirmed_category = session.get(FoodCategory, mapping.subcategory_id)
...
def _purge_expired_ai_mappings(session: Session, *, now: datetime | None = None) -> int:
    """清理已过期且未经用户确认的临时分类映射。
    ...
    result = session.execute(
        delete(ItemCategoryMapping).where(*expired_filter)
    )
