...
class ReminderService:
    ...
    async def settings(self, refrigerator_id: str, recipient_key: str) -> NotificationSettings:
        """Return persisted settings or an unsaved product-default settings object."""
        saved = await self._session.get(NotificationSettings, (refrigerator_id, recipient_key))
    ...
    async def due(self, refrigerator_id: str, recipient_key: str) -> list[DueNotification]:
        ...
        """
        settings = await self.settings(refrigerator_id, recipient_key)
        if self._now.time() < _parse_time(settings.reminder_time):
            return []
        notifications: list[DueNotification] = []
        if settings.daily_reminder_enabled:
            notification = await self._food_notification(refrigerator_id)
            if notification and await self._record_once(
                refrigerator_id, recipient_key, notification
            ):
                notifications.append(notification)
        if settings.device_health_enabled:
            notification = await self._device_health_notification(refrigerator_id)
            if notification and await self._record_once(
                refrigerator_id, recipient_key, notification
            ):
                notifications.append(notification)
    ...
    async def _food_notification(self, refrigerator_id: str) -> DueNotification | None:
        expiry = await self._session.get(ExpirySettings, refrigerator_id)
        rule = ExpiryRule(
    ...
    async def _device_health_notification(self, refrigerator_id: str) -> DueNotification | None:
        displays = list(
