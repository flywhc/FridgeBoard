...
| P5 | 库存、分类与图标库 | 待评审 | P2、P4 | 2026-08-05 首次打开补齐最近分类会话 | 最近分类读取会保留历史顺序，并从全部可用小类补齐至 16 个唯一图标；后端 76 个测试、Ruff、锁文件检查通过，待已有冰箱真机确认 2×8 无空位。 |
| P6 | 相机、条码与 AI 增量识别 | 进行中 | P1、P3、P5 | 2026-08-05 条码查询审查问题修复会话 | 处理 ZXing 单帧未找到条码、AI 无结果和请求失败后的自动重开取景；补强高分辨率、自动对焦和 `TRY_HARDER`，接入免费商品库和二维码文本大模型解析；商品二维码、网址二维码和一维条码仍待真机验收。 |
...
| P9 | 食谱、动态补货与库存扣减 | 待评审 | P2、P5 | 2026-08-05 食谱按库存食材名称匹配会话 | 食谱缺货、完成扣减和撤销恢复改为按库存 `item_name` 严格匹配，分类不再参与；后端 83 项测试、Ruff、前端 61 项测试、lint/build 和差异检查通过，待真实设备验收。 |
