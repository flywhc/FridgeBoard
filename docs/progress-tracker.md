...
- 验证：`npm run --prefix frontend lint`、`npm run --prefix frontend test -- --run`（48 passed）、`npm run --prefix frontend build`、`uv run ruff check backend`、`uv run pytest`（74 passed）、`git diff --check` 均通过；新增列表信息布局/有效期文案断言和后端默认生产日期断言。
- 未验证：真实手机触控、系统字体放大、网络断开后数量保存失败的人工体验；未进行 Playwright 视觉核验。
- 下一步：评审环境在 320/390/430px 视口人工确认列表长名称、日期/备注有无值、数量编辑和保存反馈；确认后将 P5 条目并入综合验收。
...
- 验证：`npm run --prefix frontend lint`、`npm run --prefix frontend test -- --run`（48 passed）、`npm run --prefix frontend build`、`git diff --check` 均通过。
- 未验证：真实手机触控和 320/390/430px 视口下的人工视觉表现；未进行 Playwright 视觉核验。
- 下一步：评审环境确认列表长名称、数量上下箭头和最右侧编辑箭头在窄屏下无重叠。
...
| P5 | 库存、分类与图标库 | 待评审 | P2、P4 | 2026-08-02 冰箱预览尺寸契约重构会话 | 位置独立页与弹窗统一复用 location 场景尺寸；390/430px 为 322×438、320px 为 227×309，无固有宽度收缩和横向溢出，待真机视觉评审。 |
| P6 | 相机、条码与 AI 增量识别 | 进行中 | P1、P3、P5 | 2026-07-30 添加食材二次审查修复会话 | 正在修复相机权限失败后不能自动回到表单的问题。 |
| P7 | 手机端日常首页与冰箱管理 | 待评审 | P3、P4、P5 | 2026-08-03 首页移除状态区分隔线会话 | 首页、食谱、冰箱、我的统一复用 `PageShell` 与 `P7Navigation` 应用壳；首页预览统一按最长边等比缩放；本次移除首页状态区分隔线并收紧预览顶部间距，待真机视觉评审。 |
...
| P11 | 端到端验收与发布准备 | 待评审 | P3、P6、P8、P9、P10、P10.5 | 2026-08-03 发布当前最新代码会话 | `afaab07` 已发布；数据库备份、容器健康、Alembic `20260802_11 (head)`、公网健康检查和首页 HTTP 200 均通过；待真实 PWA/冰箱端刷新验收。 |
