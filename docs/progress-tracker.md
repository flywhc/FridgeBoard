...
| P3 | 无账号配对与设备授权 | 完成 | P1、P2 | 2026-07-19 P3 配对会话 | 配对 API、PWA 页面、flycn SSO 契约与生产端到端测试 |
...
- 状态：完成
- 改动：实现短效单次 Kindle Passcode、Kindle 手机配对会话、HttpOnly 独立设备凭证、撤销与重新扫码；新增 PWA 的登录、Kindle 绑定和设备管理页。flycn 新增固定回跳地址、60 秒单次授权码和服务端兑换接口，不影响既有门户登录与发布流程。
- 验证：`uv run ruff check backend`、`uv run pytest`（17 passed）、前端 lint/build 通过；flycn `ruff check .`、`compileall`、全量测试（26 passed）通过；生产环境完成 Kindle 绑定、配对会话创建、PWA standalone 消费、当前冰箱读取、Passcode/配对令牌单次使用和非 PWA 拒绝验证。
- 未验证：目标 Kindle DP75SDI 实机浏览器兼容性、二维码扫描能力和 Web Push，保留至 P8/P11 的实机 Spike；当前 Kindle 专用展示页仍属于后续视图工作。
- 待决策：无。
- 下一步：进入 P4；Kindle 专用展示和低频同步在 P8 实现。
