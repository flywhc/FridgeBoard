...
| P5 | 库存、分类与图标库 | 待评审 | P2、P4 | 2026-07-30 数量控件与小类图标间距微调 | 保留用户已手动校准的小类图标和右箭头位置，数量区与小类图标之间的空白已收紧；按用户要求未追加浏览器验证 |
...
| P7 | 手机端日常首页与冰箱管理 | 待评审 | P3、P4、P5 | 2026-07-30 食材列表标题与搜索控件细节修复会话 | 分格标题改为用户可读的“冷藏室-1”格式，搜索框改用 20×20px 内联 SVG 放大镜并移除蓝色焦点外框；列表行为不变。 |
