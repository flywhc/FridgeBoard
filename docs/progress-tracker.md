...
| P3 | 无账号配对与设备授权 | 完成 | P1、P2 | 2026-07-23 P3 验收 | 首次冰箱端二维码、PWA 登录/本地领取、设备撤销/重配对、自动续期与 UI 验证 |
...
| P6 | 相机、条码与 AI 增量识别 | 待评审 | P1、P3、P5 | 2026-07-23 P6 实现与发布 | 相机取景页、条码识别入口、Agnes OpenAI 兼容适配与生产配置；本地构建、容器健康检查及生产 `/healthz` 已通过 |
...
| P9 | 食谱、动态补货与库存扣减 | 进行中 | P2、P5 | 2026-07-23 P9 实现会话 | 食谱 API、严格小类匹配、动态补货、完成/撤销事务、手机端本周页和导入/补货入口；待补单日编辑与浏览器视觉验收。 |
...
- 状态：进行中
- 改动：新增 `RecipeService` 和所有者食谱 API，支持固定七日读取、多行文本导入、严格小类名匹配、即时缺货、本周/下周补货清单、按最早 BBD 扣减及按原批次撤销。手机端加入食谱主导航、本周食谱、粘贴导入和可复制补货列表入口。
- 验证：`uv run pytest backend/tests/test_recipe_api.py`（2 passed）、`uv run ruff check backend`、`npm run --prefix frontend build` 通过。
- 未验证：单日食谱编辑界面与编辑 API 的端到端手验尚未完成；本地浏览器核验因开发 API 默认数据库缺少迁移返回 500 而中断，尚未产出 390×844 截图。冻结设计稿 `b2e77ba8-52dd-4722-8e89-accdf9f3569f`、`ef62678e-0a73-431a-93c6-794f646f5c74`、`bbeda1ae-e99c-40d6-87b3-90cdedd7adfa`、`903728d2-d6b9-4918-82b5-9d3ab6b3aafb` 已作为实现依据。
- 待决策：无。
- 下一步：补齐单日编辑页面和无匹配食材的可修正导入语义，使用已迁移临时 SQLite 完成 320/390/430px 截图核验后转为待评审。
