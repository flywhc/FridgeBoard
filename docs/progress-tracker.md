...
- 范围：账号跨手机恢复、PWA 实例扫码授权、未完成冰箱状态、手机空首页与统一扫码入口、布局完成后的绑定引导、冰箱设置绑定/换绑、Kindle 首次/已安装双路径提示、数据/API 影响和分阶段实施计划；不修改应用代码、数据库或运行配置。
- 设计/需求基线：用户本次讨论结论；`docs/ui-design-specification.md`；功能设计 §10；ADR-0002；现有安装、配对成功、Kindle 二维码、创建布局和冰箱设置设计资产。
- 完成：新增 `docs/pairing-and-onboarding-redesign.md`，明确账号拥有与设备访问的权限边界、PWA `/` 启动地址、六类冰箱/布局/设备状态、三处扫码入口、未完成设置恢复、单活跃冰箱端与原子换绑、逐场景提示语、领域/API 影响、六阶段实施和真实设备验收矩阵；同步功能设计、冰箱设置需求和最终设计注册表的待补资产。
- 验证：人工交叉检查新设计与现有 Manifest、功能设计 §10、ADR-0002、P7.1 冰箱设置需求及当前前后端配对模型；`git diff --check` 通过。
- 未验证：尚未生成或确认新流程的最终 UI 设计资产；尚未验证数据库迁移、接口兼容、真实 iOS/Android PWA 和 DP75SDI Kindle 流程。
- 下一步：产品确认“一台冰箱一个活跃冰箱端”、六位码降为兼容路径和非所有者扫码仅获得实例访问权；随后补齐设计资产并按计划阶段 1 先完成领域模型、API 契约和失败测试。
...
| P9 | 食谱、动态补货与库存扣减 | 待评审 | P2、P5 | 2026-08-06 零数量软删除修复会话 | 食谱完成归零后保留原日期，撤销恢复数量后重新显示日期和提醒；手工改数与食谱扣减路径已区分；全量后端 85 项、前端 66 项测试及 lint/build 通过，待真实设备验收。 |
