...
- 会话记录：已确认现有实现仅按横向等分点排列，并通过固定 `±6px` 垂直偏移交错；改为二维归一化坐标、按横格/竖格选择网格，以及 CSS 2px 内缩定位。
- 完成：1/2/3/4/5 个图标分别使用中心、左右三等分、三角形、四点网格和中心加四角；更多图标使用方向感知的二维网格，不完整行居中，图标多时天然形成上下左右错位；图标主体定位轨道距分格外框至少 2px。
- 验证：`npm run --prefix frontend test -- --run`（4 个测试文件、68 passed）、`npm run --prefix frontend lint`、`npm run --prefix frontend build`、`git diff --check` 均通过；已核对冻结首页本地资产 `docs/ui-assets/png/pwa-home.png` 与 `docs/ui-assets/html/pwa-home.html`。
- 未验证：真实手机/PWA 视觉核验及不同设备缩放下的人工确认。
- 下一步：在 320/390/430px 设备上抽样确认竖格 3–8 个图标的填充、2px 留白和高密度错位效果；确认后可标记完成。
