...
拟物冰箱结构使用手机端与 Kindle 共用的 `fridgeLayoutCore.js` 生成渲染计划。该源文件保持 ES5，并由 Vite 原样输出为 `/fridge-layout-core.js`；Kindle 页面先加载共享核心，再加载仅负责食物图标分散的 `kindle-layout.js` 和页面 DOM 渲染器。不得在 Kindle 资源中再次复制外壳尺寸、柜体分带、门分段或合页算法。
