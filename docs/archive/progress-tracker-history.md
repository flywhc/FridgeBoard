...
### 2026-08-29 — 修复 Android 首次使用页登录按钮多余材质（本次会话）

- 状态：待评审。
- 目标：移除 Android APK 安装后“开始使用家常食橱”页面“登录或注册”图片按钮多余的套圈和白色背景，保留按钮图片本体、文字、点击行为和最小触控热区。
- 范围：`frontend/src/styles.css` 及必要的前端样式回归断言和本进度记录；不改变登录流程、其他页面按钮材质、用户图片资源或主题选择逻辑。
- 设计/需求基线：用户本次明确反馈；`docs/ui-design-specification.md`；`docs/functional-design-and-feasibility.md` §10、§17.1；`docs/final-ui-designs.md` 已确认的首次使用本地设计板 `docs/ui-assets/proposals/pairing-onboarding-redesign.{html,png}`。
- 预期验证：拟物主题首次使用页的 `.pairing-secondary` 不再渲染额外图片套圈、白色底和椭圆投影，水墨/卡通主题与其他按钮保持现状；运行前端测试、lint、生产构建和 `git diff --check`。
- 会话记录：已确认 Android 安装后默认使用拟物主题；首次使用页的登录按钮复用通用拟物按钮规则，元素本身和通用规则均会叠加背景/边框/盒阴影，且通用 `::after` 会绘制底部椭圆投影。本次保留 `secondary-master.webp` 图片伪元素，仅按页面和按钮类清除外层背景、边框、盒阴影及椭圆伪元素。
- 完成：新增 `[data-theme="skeuomorphic"] .pairing-empty-content .pairing-secondary` 覆盖，外层背景、边框和盒阴影透明；其 `::after` 背景透明并隐藏；登录文字、点击处理、图片底色和 48px 最小热区未改变。补充 `frontend/src/App.test.ts` 样式契约断言。
- 验证：`npm run --prefix frontend test -- --run src/App.test.ts`（165 passed）、`npm run --prefix frontend test -- --run`（36 个测试文件、371 passed）、`npm run --prefix frontend lint`、`npm run --prefix frontend build` 和 `git diff --check` 均通过。
- 未验证：未在真实 Android APK 安装后的 Capacitor WebView 中重新打开首次使用页并进行人工视觉/触摸复测；未提交 Git、未发布生产。
- 下一步：在 Android 设备安装包含本次前端构建的 APK，确认默认拟物主题下“登录或注册”仅显示图片按钮底色，无额外套圈、白色外层或椭圆投影，且可正常点击登录。

### 2026-08-29 — 补充 Android 正式签名材料位置文档（本次会话）

- 状态：完成。
- 完成：在 `README.md` 和 `docs/mobile-deployment-design.md` 记录当前开发机 keystore、Gradle 配置文件、alias、SHA-256 指纹、权限要求及本地构建注入命令；未记录密码或密钥内容。
- 验证：已确认文档中的路径为 `/Users/jason/secure/fridgeboard-release.jks` 与 `/Users/jason/secure/fridgeboard-keystore.properties`，两文件权限均为 `600`；`git diff --check` 通过。

### 2026-08-29 — Android Debug 签名迁移与自动更新真机验收（本次会话）

- 状态：完成。
- 目标：调查 Android Debug APK 检查更新/下载安装失败原因，发布开发版并在真实设备完成下载更新验收。
- 根因：设备原安装包使用 Android Debug 证书 `798501f3...16ea3`，服务器 APK 使用正式证书 `bcc72627...e9d0a`，因此直接覆盖安装返回 `INSTALL_FAILED_UPDATE_INCOMPATIBLE`；本地 Gradle 未注入外部签名属性时，`debug` 构建会回退到 Debug 证书。
- 处理：使用 `/Users/jason/secure/fridgeboard-keystore.properties` 注入正式签名；因签名迁移必须对设备原 Debug 安装做一次完整卸载；以 `versionCode=1700000009` 安装签名基线后，重新发布同版本 `0.1.7` 的开发构建 `release=260829024900`、`versionCode=1700000010`。
- 完成：GitHub Actions run `33200989293` 成功，服务器最新元数据、独立下载文件和 APK 内签名/版本信息一致；设备通过应用内原生下载与系统安装器完成更新，最终为 `versionName=0.1.7`、`versionCode=1700000010`，安装包无 `DEBUGGABLE` 标记，更新文件大小 `6778670` 字节。
- 验证：已复现旧 Debug 包覆盖安装失败；已验证正式证书指纹与本地 keystore、服务器 APK 一致；已执行 `scripts/mobile-release.sh build`、GitHub Actions、服务器 metadata/下载 SHA-256 校验及真机 `downloadAndInstallApk` 全链路测试，设备最终安装成功。测试期间开启的 Android 包校验设置已恢复。
- 未验证：未在第二台 Android 设备重复迁移；现有 Debug 安装用户仍需先卸载一次再安装正式签名版本，之后才能正常覆盖更新。
- 下一步：后续本地 Debug/开发版构建必须显式设置 `FRIDGEBOARD_ANDROID_KEYSTORE_PROPERTIES=/Users/jason/secure/fridgeboard-keystore.properties`，正式发布继续使用 GitHub Secrets 注入签名。
...
- 预期验证：在线和 AI 候选共用加载动画；资源成功后 DOM 只保留实际图片；减少动画偏好下动画可访问地降级；前端测试、lint、生产构建和 `git diff --check` 通过。
- 会话记录：已确认 AI 生成中的活动候选已有局部动画，但 `RuntimeImage` 在受保护资源 Blob 完成前返回空 `src`，因此 AI 候选和在线候选的加载阶段表现不一致。
- 未完成：尚未实现统一占位组件和测试。
- 下一步：修改 `RuntimeImage` 及其样式，补充在线与 AI 加载状态回归断言，再运行前端门禁。
...
- 状态：待评审。
- 目标：修复“我的冰箱”底部“最近删除”入口缺失左右留白和圆角的问题，并重新审计所有页面级拟物内容入口，避免遗漏同构布局错误。
- 范围：`frontend/src/styles.css`、前端回归契约和本进度记录；不改变最近删除导航、删除恢复业务或有意的图标/文字/危险操作控件样式。
- 设计/需求基线：用户本次视觉反馈；`docs/ui-design-specification.md` §4.4、§5、§7；`docs/final-ui-designs.md` P7.1 及主题系统本地视觉基线。
- 预期验证：最近删除与临期规则、名称与布局共享拟物内容卡片的左右留白、圆角、背景和阴影；全量前端测试、lint、生产构建和 `git diff --check` 通过。
- 会话记录：复查确认 `.p71-deleted-link` 仍使用旧的全宽透明分隔线规则，上一轮同类布局审计漏掉了该入口；本轮同步核对页面级链接、列表行和卡片，保留返回、图标、开关、步进器、文字加号和危险操作等明确的非卡片例外。
- 完成：为拟物主题 `.p71-deleted-link` 补齐与 `.p7-link-row`、`.p71-name-layout-link` 一致的 `12px` 左右留白、`8px` 圆角、页面表面背景和拟物阴影；复查确认其它页面级入口未发现新的同类漏项，透明控件均为已登记的设计例外。
...
- 未验证：未在真实 Android WebView/PWA 中逐页截图复测；未提交或发布 Git。
- 下一步：评审时切换拟物主题打开“我的冰箱”，确认“最近删除”与“名称与布局”“临期规则”具有一致的留白、圆角、背景和阴影。
...
- 预期验证：从生产日志确认实际请求状态和异常上下文；补充能复现该回归的前端/后端测试；运行相关前端测试、lint、build，后端 Ruff/pytest（触达后端时）和 `git diff --check`；明确线上部署/真机验证是否执行。
