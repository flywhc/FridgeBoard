...
- PWA 离线启动由缓存应用壳立即返回静态 splash；联网导航优先取得服务器上的新版 `index.html`。Service Worker 安装时同时缓存入口 HTML 及其引用的 hashed JS/CSS，激活后重新导航已打开的 PWA 窗口，避免旧入口资源已删除时页面内 release 同步无法执行。缺失的 `/assets/*` 保持真实 404，不得由 PWA 路由回退成 HTML。release 同步在 splash 阶段显示“正在更新...”，检查、注册或清理失败均放弃本次升级并继续进入主界面。
