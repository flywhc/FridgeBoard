...
移动端产品版本的唯一来源是 `frontend/package.json` 的 `version`，当前为 `0.2.2`，格式固定为三段数字
...
正式发布使用单一编排入口：先确定一个 UTC `yymmddhhMMss` release，创建并推送 `v<version>` tag，再将同一 release 显式传给 [`scripts/deploy-image.sh`](../scripts/deploy-image.sh) 和 Android `workflow_dispatch`；服务器部署成功后只触发一次 APK workflow。发布脚本最后调用受独立 `FRIDGEBOARD_ANDROID_RELEASE_CACHE_TOKEN` 保护的 `POST /api/internal/android/releases/cache/clear`，再校验同域更新接口的版本、release 和构建号。该 token 不得复用 Flycn 客户端密钥，也不得放入 Git、命令行参数或日志。

Android Release 元数据服务使用进程内缓存，默认 TTL 为 5 分钟。缓存清除接口不在 OpenAPI 暴露，缺少服务端 token 时返回 404，token 错误返回 401，成功时仅清除该进程的 Android Release 元数据缓存并记录操作日志；它不清除用户数据、数据库或其他业务缓存。单容器部署下该接口立即生效；未来扩容时必须改为共享缓存或逐实例清除。
