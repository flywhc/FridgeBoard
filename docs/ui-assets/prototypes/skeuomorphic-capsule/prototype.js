...
document.querySelectorAll('.top-action').forEach((button) => {
...
function navFooter(active) {
  ...
  return `<nav class="capsule capsule--bottom" id="bottomCapsule" aria-label="主导航"><div class="capsule-skin" aria-hidden="true"><img class="slice-left" src="assets/bottom-warm-transparent-left.png" alt=""><img class="slice-center" src="assets/bottom-warm-transparent-center.png" alt=""><img class="slice-right" src="assets/bottom-warm-transparent-right.png" alt=""></div><div class="bottom-content">${entries.map(([key, icon, label]) => `<button class="nav-item${key === active ? ' is-active' : ''}" type="button" data-nav="${key}">${icon}<span>${label}</span></button>`).join('')}</div></nav>`;
}
...
function seamSize(capsule) {
  if (!capsule) return 0;
  const rects = [...capsule.querySelectorAll('.capsule-skin img')].map((slice) => slice.getBoundingClientRect());
  return Math.max(0, rects[1].left - rects[0].right, rects[2].left - rects[1].right);
}
...
function updateMetrics() {
  ...
  const topSlice = topCapsule.querySelector('.slice-left');
  const cap = topSlice ? topSlice.getBoundingClientRect() : null;
  ...
  setMetric('topSeam', cap ? `${seamSize(topCapsule).toFixed(2)}px` : '不适用');
  setMetric('bottomSeam', bottomCapsule ? `${seamSize(bottomCapsule).toFixed(2)}px` : '不适用');
  setMetric('capRatio', cap ? (cap.width / cap.height).toFixed(4) : '不适用');
